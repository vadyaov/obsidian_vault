Из [[Вывод типа шаблона | предыдущего раздела]]можно узнать почти все, что следует знать о выводе типа `auto`, поскольку за одним люблпытным исключением вывод типа `auto` *представляет собой* вывод типа шаблона.
